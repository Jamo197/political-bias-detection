...
def load_rag_logs(base_dir: Path) -> pd.DataFrame:
    ...
    if not jsonl_files:
        raise FileNotFoundError(f"No party_label_*.jsonl files found under '{base_dir}'")
...
def compute_score_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    feats = df["scores"].apply(lambda s: pd.Series(_features(s), index=["score_top1", "score_top5", "score_mean", "score_variance", "score_range"]))
    ...
    for (emb, mode), g in df.groupby(["embedding_model", "retrieval_mode"]):
        ...
        if denom > 0:
            df.loc[g.index, "score_mean_normalized"] = (g["score_mean"] - cond_min) / denom
        else:
    ...
    for (emb, mode), g in df.groupby(["embedding_model", "retrieval_mode"]):
        ...
        for idx, row in g.iterrows():
            ...
            df.at[idx, "score_variance_normalized"] = float(np.var(norm, ddof=1)) if len(norm) >= 2 else 0.0
    ...
    df = df.merge(valid[["run_id", "text_index", "embedding_model", "retrieval_mode", "abs_error"]],
        ...
        groups.append(
            {
                ...
                "median_variance_norm": round(g["score_variance_normalized"].median(), 4),
...
def compute_correlations(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def _corr(sub: pd.DataFrame, label: str, score_col: str = "score_mean") -> Dict:
        if len(sub) < 3:
            return {"group": label, "score_type": score_col, "n": len(sub),
...
def plot_boxplot_all_scores(df: pd.DataFrame, out_path: Path, normalized: bool = False):
    ...
    fig, ax = plt.subplots(figsize=(12, max(6, len(plot_df["condition"].unique()) * 0.5)))
    sns.boxplot(data=plot_df, x="score", y="condition", ax=ax, palette="mako", hue="condition", legend=False)
    title = "Distribution of Retrieval Scores (All Chunks)" + (" — Normalized" if normalized else "")
    ax.set_title(title, fontsize=14, fontweight="bold")
    ax.set_xlabel("Normalized Score [0,1]" if normalized else "Cosine Similarity Score", fontsize=12)
    ax.set_ylabel("Condition (Embedding / Strategy)", fontsize=12)
    _save(fig, out_path)


def plot_boxplot_metric(df: pd.DataFrame, metric: str, title: str, xlabel: str, out_path: Path):
    """Generic box plot of a per-query metric across conditions."""
    sns.set_theme(style="whitegrid")
    fig, ax = plt.subplots(figsize=(10, max(6, len(df["condition"].unique()) * 0.5)))
    sns.boxplot(data=df, x=metric, y="condition", ax=ax, palette="mako", hue="condition", legend=False)
    ax.set_title(title, fontsize=14, fontweight="bold")
...
def plot_score_vs_error(df: pd.DataFrame, out_path: Path, use_normalized: bool = False):
    ...
    if valid.empty:
        print(f"Skipping {'normalized ' if use_normalized else ''}score-vs-error plot — no valid data.")
        return
    ...
    for idx, cond in enumerate(conditions):
        ...
        sns.scatterplot(data=sub, x=score_col, y="abs_error", alpha=0.5, ax=ax, edgecolor=None)
        sns.regplot(data=sub, x=score_col, y="abs_error", scatter=False, ax=ax, color="red", line_kws={"linewidth": 1.5})

        pr, pp = pearsonr(sub[score_col], sub["abs_error"])
        sr, sp = spearmanr(sub[score_col], sub["abs_error"])

        ax.set_title(cond, fontsize=11, fontweight="bold")
        ax.set_xlabel("Normalized Mean Score [0,1]" if use_normalized else "Mean Retrieval Score", fontsize=10)
    ...
    title = "Score-to-Error Calibration: " + ("Normalized " if use_normalized else "") + "Mean Retrieval Score vs. Absolute Error"
...
def main(base_dir: Path = BASE_DIR, output_dir: Path = OUTPUT_DIR):
    ...
    # Save per-query CSV
    per_query_cols = [
        "run_id", "model", "embedding_model", "retrieval_mode", "condition",
        "text_index", "party", "n_chunks",
        "score_top1", "score_top5", "score_mean", "score_variance", "score_range",
        "score_mean_normalized", "score_range_normalized", "score_variance_normalized",
        "predicted_bias", "label_ideology", "abs_error",
    ]
    ...
    plot_boxplot_metric(df, "score_range", "Score Range (Top1 − Top5) per Query", "Score Range", output_dir / "boxplot_score_range.png")
    plot_boxplot_metric(df, "score_variance", "Score Variance per Query", "Variance", output_dir / "boxplot_score_variance.png")
    plot_score_vs_error(df, output_dir / "scatter_score_vs_error.png", use_normalized=False)

    # Plots — normalized
    print("\nGenerating normalized-score plots …")
    plot_boxplot_all_scores(df, output_dir / "boxplot_all_scores_normalized.png", normalized=True)
    plot_boxplot_metric(df, "score_range_normalized", "Normalized Score Range per Query", "Normalized Range [0,1]", output_dir / "boxplot_score_range_normalized.png")
    plot_boxplot_metric(df, "score_variance_normalized", "Normalized Score Variance per Query", "Normalized Variance", output_dir / "boxplot_score_variance_normalized.png")
    plot_score_vs_error(df, output_dir / "scatter_score_vs_error_normalized.png", use_normalized=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--base-dir", type=Path, default=BASE_DIR, help="Root directory with batch run JSONL files.")
    parser.add_argument("--output-dir", type=Path, default=OUTPUT_DIR, help="Directory to write results.")
